...
public class NewModelV2 {
    private static final Logger log = LoggerFactory.getLogger(NewModelV2.class);
	...
	if(args.length != 14){
	    System.out.println("Usage: sampleInterval(Int) walkerSpeed(Double) busCruisingSpeed(Double) "
	    	...
	    	+ "eps4ST(Double) minPts4ST(Int) /path/to/GPS_DATA_ROUTE /path/to/BUS_ROUTE.csv /path/to/save/RESULT_STOP_LIST.csv");
	    System.exit(0);
	}
	...
	GPSCsvPath = args[11];
	BusRouteCsvPath = args[12];
	ResultStopListPath = args[13];
	
	long bt = System.currentTimeMillis();
	BRMapper brMapper = new BRMapper(BusRouteCsvPath);
	Map<Integer,Integer> BusRouteMap = brMapper.getBusRouteMap();
	
	TransitCenter transitCenter = NewModelV2.constructCenter(GPSCsvPath, BusRouteMap);
	...
	for(int route_id:routeIdList){
	    ...
	    Station[] terminals = NewModelV2.getStations(BTObjects, GPSKDTree);
	    ...
	    log.info(routeSum+ " routes left. "+(rft-bt)/1000+" seconds spent total. "+invalidCount+" routes are skipped.");
	}
